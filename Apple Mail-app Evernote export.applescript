...
on error errText number errNum --ERROR HANDLING
	display dialog "An outer error occurred" & return & "error number " & errNum & return & errText
...
on attachment_process(thisMessage, theNote)
	local exportFolder
	tell application "Mail"
		...
		set exportFolder to my getExportFolder("Temp Export From Mail")

		--PROCESS THE ATTACHMENTS
		set theAttachments to thisMessage's mail attachments
		set attCount to 0
		repeat with theAttachment in theAttachments
			set theFileName to (exportFolder as text) & theAttachment's name
			try
		...
		return my moveFolderToTrash(exportFolder)
	end tell
end attachment_process

--Get the export folder by the given name (in a temporary location), creating it if it does not exist
on getExportFolder(exportFolderName)
	local tempFolder, exportFolder
	set tempFolder to alias "tmp" --usually resolves to /private/var
	try
		--try to get alias, will throw error if folder does not exist
		set exportFolder to alias ((tempFolder as text) & exportFolderName & ":")
	on error
		tell application "Finder"
			set exportFolder to (make new folder at tempFolder with properties {name:exportFolderName})
		end tell
	end try
	return exportFolder
end getExportFolder

-- Move the given folder to the trash. If there is a folder by the same name in the trash, a new name
-- is generated by appending number a number that is incremented until no file or folder by the new
-- name exists.
-- (original comment: SILENT DELETE OF TEMP FOLDER (THANKS MARTIN MICHEL!))
on moveFolderToTrash(saveLoc)
	try
		local counter, saveLocAlias, saveLocName, saveLocPath, destFolderAlias, destFolderPath, trashFolderPath
		set saveLocAlias to saveLoc as alias
		set trashFolderPath to (path to trash) as text
		set saveLocName to name of (info for saveLocAlias)
		set saveLocPath to quoted form of POSIX path of saveLocAlias
		set counter to 0
		set destFolderPath to trashFolderPath & saveLocName & ":"
		repeat
			try
			...
			set destFolderPath to trashFolderPath & saveLocName & " " & counter & ":"
		end repeat
		set destFolderPath to quoted form of POSIX path of destFolderPath
		set command to "mv " & saveLocPath & space & destFolderPath
		do shell script command
		return true
	on error
		return false
	end try
end moveFolderToTrash
